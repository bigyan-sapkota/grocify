...
export default function PendingItemCard({ item }: PendingItemCardProps) {
  const { togglePurchased, updateQuantity, removeItem } = useGroceryStore();
  return (
    <View className="rounded-3xl border border-border bg-card p-4">
      <View className="flex-row items-start gap-3">
        <Pressable
          className="mt-1 h-6 w-6 items-center rounded-full border-2 border-border bg-card justify-center"
